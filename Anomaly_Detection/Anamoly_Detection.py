...
Flags out-of-sample days where the HMM's regime posterior shows
low confidence / high uncertainty, based on three signals:
  - posterior_top_prob : probability of the most likely state
  - posterior_margin    : gap between top and 2nd-most-likely state
  - posterior_entropy   : entropy of the full posterior distribution

Also visualizes the anomalies against the cumulative return / regime chart.

REQUIREMENT:
    This script expects a CSV file (default: hmm_oos_results.csv) in the
    same folder, containing at minimum these columns:
        posterior_top_prob, posterior_margin, posterior_entropy, regime
    and ideally a cumulative return column (cum_strategy / strategy_cum /
    cum_hardcap) and/or 'close' for the price chart overlay.

    If the file is missing, the script will print a clear error and exit
    instead of crashing — you need to place/generate that CSV first.
